...
params = ['hidden_size', 'graph_skip_conn',
          'update_adj_ratio', 'smoothness_ratio', 'degree_ratio','sparsity_ratio', 'graph_learn_num_pers', 'learning_rate', 'weight_decay']
...
class ModelHandler(object):
    ...
    def __init__(self, config):
        ...
        if not os.path.exists(log_folder):
            os.mkdir(log_folder)
        log_path=log_folder+config['dataset_name']+'.txt'
        ...
        if config.get('use_random_seed',False):
            seed=np.random.randint(1,10000)
            print_str = f'generating random seed {seed}\n'
            print(print_str)
