...
def load_data( dataset_str, knn_size=None, epsilon=None, knn_metric='cosine', prob_del_edge=None,
              prob_add_edge=None, seed=1234, sparse_init_adj=False):
    """
    ...
    if dataset_str.startswith('ogbn'):  # Open Graph Benchmark datasets
        ...
        dataset = NodePropPredDataset(name=dataset_str, root='../data')

        split_idx = dataset.get_idx_split()
        idx_train, idx_val, idx_test = torch.LongTensor(split_idx["train"]), torch.LongTensor(
            split_idx["valid"]), torch.LongTensor(split_idx["test"])
    ...
    elif dataset_str.startswith('fb'):
        sub_name = dataset_str.strip().split('_')
        assert len(sub_name) == 2
        sub_name = sub_name[1]
        adj, labels, features, idx_train, idx_val, idx_test = load_fb100(sub_name)
        ...
        raw_features = sp.vstack((allx, tx)).tolil()  # to linked list format
