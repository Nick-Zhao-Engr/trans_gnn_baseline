...
def prepare_datasets(config):
    ...
    if config['data_type'] == 'network':
        ...
        adj, features, labels, idx_train, idx_val, idx_test = data_conf.load(data_dir=config.get('data_dir', None), knn_size=config['input_graph_knn_size'], epsilon=config.get('input_graph_epsilon', None), knn_metric=config.get('knn_metric', 'cosine'),node_remove=config.get('node_remove',0))
