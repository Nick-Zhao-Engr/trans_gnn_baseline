...
def load_fb100(dataset_str):
    ...
    labels_raw = pkl.load(open(os.path.join(root_dir, 'labels.pkl'), 'rb'))

    # print(type(adj))  # csr_matrix
    # print(type(features)) # np array
    n_class,n_node=labels_raw.max()+1,labels_raw.shape[0]
    labels=np.zeros((n_node,n_class))
    labels[np.arange(n_node),labels_raw]=1
...
def load_data(dataset_str):
    ...
    test_idx_reorder = parse_index_file(
        "{}/ind.{}.test.index".format(data_path, dataset_str))
    ...
    labels = np.vstack((ally, ty))  #(num_node, n_class)
    labels[test_idx_reorder, :] = labels[test_idx_range, :]

    # idx_test = test_idx_range.tolist()
    # idx_train = range(len(y))
    # idx_val = range(len(y), len(y)+500)
    idx_train = pkl.load(open(os.path.join(data_path, 'idx_train.pkl'), 'rb'))
    idx_val = pkl.load(open(os.path.join(data_path, 'idx_val.pkl'), 'rb'))
    idx_test = pkl.load(open(os.path.join(data_path, 'idx_test.pkl'), 'rb'))
